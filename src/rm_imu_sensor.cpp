...
namespace gary_hardware {
    ...
    hardware_interface::return_type RMIMUSensor::configure(const hardware_interface::HardwareInfo &info) {
        ...
        RCLCPP_DEBUG(rclcpp::get_logger("rm_imu_system"), "using orientation can id %d", this->can_ids[0]);

        //bind orientation can id
        can_receiver->bind(this->can_ids[0]);
        ...
        this->can_ids[2] = std::stoi(info.hardware_parameters.at("gyro_can_id"), nullptr, 16);
        RCLCPP_DEBUG(rclcpp::get_logger("rm_imu_system"), "using gyroscope can id %d", this->can_ids[1]);

        //bind gyroscope can id
        this->can_receiver->bind(this->can_ids[1]);
        ...
        this->can_ids[1] = std::stoi(info.hardware_parameters.at("accel_can_id"), nullptr, 16);
        RCLCPP_DEBUG(rclcpp::get_logger("rm_imu_system"), "using acceleration can id %d", this->can_ids[2]);

        //bind acceleration can id
        this->can_receiver->bind(this->can_ids[2]);
